(ns mofcompliance.operation
  "OperationActor -- one MOF compliance operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (ProcurementCompliance-LLM) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the MOF Procurement Compliance
  ...
  governor is clean -- see `mofcompliance.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [mofcompliance.mofcompliancellm :as mofcompliancellm]
            [mofcompliance.governor :as governor]
            [mofcompliance.phase :as phase]
...
(defn build
  ...
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (mofcompliancellm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (mofcompliancellm/-advise advisor store request)]
            {:proposal p :audit [(mofcompliancellm/trace request p)]})))
