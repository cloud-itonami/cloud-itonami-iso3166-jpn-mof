...
;; The agency hosts serve a bot challenge when asked too often -- the sibling
;; METI register measured exactly that against its hosts, and nothing here says
;; the MOF-side hosts are immune. A mutation suite is exactly the tight loop
...
;; in six seconds did it on the METI hosts 2026-08-26.
...
(defn- tmpfile [suffix]
  (path/join (os/tmpdir) (str "mof-mutation-" suffix ".edn")))
...
(def laws-only
  ...
  that pushes back: it answered dozens of consecutive lookups while this
  register was built, whereas agency hosts here and elsewhere serve a bot
  challenge when asked too often. Testing the law branch against a page-bearing
  register would put four needless agency requests behind each mutation."
  (subset #{"law.fefta"}))
...
(def with-pages
  "One law plus the three measured hosts, three :page-identity entries across
  two hosts (so the self-test pool has two hosts and a spare probe), and one
  :page-text entry -- the smallest register that still reaches the page
  branches.

  customs.home is included BECAUSE its :page/charset excludes it from the
  self-test pool: the page mutations below are aimed at it precisely so that
  a mutated entry is never also a self-test probe. A probe that carries the
  mutation under test drives the real check functions against a broken page
  and comes back with the WRONG reason -- observed 2026-08-29 as four
  WRONG-EXIT results when the mutations pointed at pool entries."
  (subset #{"host.www-mof-go-jp" "host.www-customs-go-jp" "host.www-p-portal-go-jp"
            "law.fefta" "mof.home" "portal.home" "portal.spa" "customs.home"
            "mof.invest-japan"}))
  ...
  [{:id "nonexistent-law" :on :laws-only
   ...
   {:id "fail-outranks-refusal" :on :laws-only
    ...
    :why (str "a register with a real finding AND an unreachable source at the "
              ...
              "found wrong, so it must not turn exit 1 into exit 2. Observed on "
              "the sibling METI register 2026-08-29, where it did -- and one "
              "intermittently blocked page hid a register error for as long as "
              "it stayed blocked.")
    :mutate (fn [d]
              (recount
                (-> d
   ...
   {:id "page-title-drift"
    ...
    :mutate #(alter-entity % "customs.home" (fn [e] (assoc e :page/title "経済産業省")))
    :want-exit 1 :want-reason "title-drift"}

   {:id "missing-text"
    :why "a string the page does not contain"
    :mutate #(alter-entity % "mof.invest-japan"
                           (fn [e] (assoc e :page/must-contain ["この文字列は実在の官庁ページには存在しない"])))
    :want-exit 1 :want-reason "missing-text"}

   {:id "charset-drift"
    :why "a Shift_JIS page declared as UTF-8 -- every Japanese string would be compared against mojibake (and the mirror-image mutation, a UTF-8 page declared Shift_JIS, is caught by the same branch; the charset self-test drives it against a UTF-8 probe)"
    :mutate #(alter-entity % "customs.home" (fn [e] (assoc e :page/charset "utf-8")))
    :want-exit 1 :want-reason "charset-drift"}

   {:id "unexpected-redirect"
    :why "a URL that lands somewhere other than where it asked"
    :mutate #(alter-entity % "customs.home"
                           (fn [e] (assoc e :source/url "http://www.customs.go.jp/")))
    :want-exit 1 :want-reason "unexpected-redirect"}

   {:id "not-2xx"
    :why "a cited page that is not there"
    :mutate #(alter-entity % "customs.home"
                           (fn [e] (assoc e :source/url "https://www.customs.go.jp/no-such-zzz.html")))
    :want-exit 1 :want-reason "not-2xx"}

   {:id "host-status-drift"
    :why "a host whose refusal is not the refusal the register measured"
    :mutate #(alter-entity % "host.www-customs-go-jp"
