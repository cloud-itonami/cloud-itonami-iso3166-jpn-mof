...
;;
;; THE CHALLENGE THAT MAY STAND IN FRONT OF A PAGE IS A 2xx. THIS FILE IS
;; BUILT SO THAT IT CANNOT PASS AS A PAGE.
;;
;; The sibling METI register measured 2026-08-26 that www.meti.go.jp, behind
;; AWS WAF, serves a 2468-byte JavaScript challenge carrying
;; window.awsWafCookieDomainList and window.gokuProps in place of the page --
;; answering HTTP **202** (a 2xx) AT the requested URL with no redirect and an
;; EMPTY <title>. Every layer of an ordinary page check reads that as success
;; except the title. A verifier that stops there calls it title drift and
;; exits 1, reporting "the register is wrong" about a run that never reached
;; the authority. The register is not wrong; the run could not answer. So the
;; challenge is detected by its own signature BEFORE the status branch, and
;; reported as :challenge-interposed with verdict :refused -- exit 2.
;;
;; No host cited in the MOF register measured on 2026-08-29 served that
;; challenge -- but that is one cold client on one day, and the detector stays
;; in regardless: a challenge is a declared refusal (exit 2), never a page
;; (exit 0 or 1).
;;
;; FETCHES ARE SERIAL AND PACED, AND THAT IS A MEASUREMENT, NOT A STYLE.
;; Concurrency is what TRIPPED the WAF on the METI hosts: measured there, a
;; cold client read real pages and roughly ten requests tripped the challenge.
;; A concurrent verifier would reliably produce the failure it is meant to
;; detect. Pages are therefore fetched one at a time with --pace milliseconds
;; between them.
;;
;; THE HOSTS CITED HERE REFUSE UNKNOWN PATHS WITH DIFFERENT STATUSES, AND EACH
;; STATUS IS PINNED. www.mof.go.jp and www.customs.go.jp answer 404 for a path
;; that does not exist; www.p-portal.go.jp answers 401 with a system-error
;; page. All are non-2xx, so status discriminates -- but a checker looking for
;; 404 specifically, or one reading 401 as "blocked, cannot measure", would
;; misreport hosts that answered perfectly clearly. The status each host
;; actually returned is pinned in :host/missing-status and compared on every
;; run. The customs front page is served Shift_JIS with no charset on the
;; Content-Type line -- the declaration sits in a meta tag -- so :page/charset
;; pins it, and the run FAILS with charset-drift if the host ever switches
;; encoding, rather than silently re-decoding every Japanese string.
;;
...
;; citing a non-UTF-8 page in the probe pool: the one Shift_JIS page the
;; register cites (customs.home) already declares its own charset, and the
;; probe pool wants pages the register expects to READ as UTF-8. The challenge
;; self-test runs against a fixture of the real challenge body rather than
;; against the live WAF, because whether a WAF is engaged at any moment is not
;; under this run's control -- a self-test that only discriminates when a third
;; party happens to be blocking us is a self-test that passes by not running.
;;
;; THE SELF-TESTS THAT DRIVE PAGES ARE ORDERED FOR THE POOL, AND THE ORDER IS
;; LOAD-BEARING HERE. The pool hands out :page-identity candidates interleaved
;; across hosts, challenge-prone hosts last. In THIS register the pool is
;; [mof.home, portal.home] -- and portal.home is a REDIRECTED SPA entry: its
;; declared :page/redirects-to would make the redirect self-test's mismatch
;; disappear, because the http:// form of its URL lands on exactly the URL the
;; register says it redirects to. So the redirect self-test takes pick 0
;; (mof.home, a plain URL with no declared redirect -- its http:// form lands
;; on the https URL, which is not what it asked for), and portal.home gets the
;; title and charset self-tests, which a declared redirect does not touch.
;;
...
;; the page. Kept from the METI register on purpose: the challenge fixture
;; describes a THIRD PARTY's block, not this register's hosts.
...
;; Measured while building the sibling METI register: three different prose
;; markers were tried and all three collided -- "challenge-interposed" appears
;; in the challenge self-test's SUCCESS line, "REFUSED[challenge-interposed]"
;; misses a challenge that blocks a self-test rather than a source, and "a bot
;; challenge" appears in the text of an unrelated guard EXPLAINING what trips
;; one. Prose written to be read by people is not a machine interface.
(def blocked (atom 0))
...
;; "this run established nothing about that source". Counted separately rather
;; than folded in, because "a third party is refusing us" and "we could not get
;; there" are different things to go and look at.
...
(defn- self-test-challenge []
  ...
  (p/resolved
    (if (challenge? CHALLENGE-FIXTURE)
      (if (challenge? "<html><head><title>税関 Japan Customs</title></head><body>輸入申告</body></html>")
        {:ok false
...
(defn- self-test-redirect [probe]
  ...
  ;;
  ;; The probe handed in here MUST be a plain-URL entry (no
  ;; :page/redirects-to), otherwise the declared redirect target would match
  ;; where it lands and the mismatch would disappear. In THIS register that is
  ;; mof.home -- see the self-test invocation order at the bottom of this file.
...
(defn- serially
  ...
  the challenge on these hosts, so this is load-bearing."
  [f xs]
...
(let [data (read-facts)]
  ...
  (let [entities (filterv map? data)
        ...
        ;;
        ;; Measured 2026-08-26 on the sibling METI register: an earlier
        ;; revision drove all four against a single probe, which asked one page
        ;; for the fifth time in about six seconds and tripped that host's AWS
        ;; WAF. The run then refused -- the right answer, but produced by the
        ;; verifier's own traffic, and no run could establish anything about
        ;; the register. So the candidates are interleaved by host and handed
        ;; out one per self-test.
    ...
    (cond
      ...
      ;; register fetch from agency hosts it never cites.
        ...
        (cond
          ...
          (p/let [selfs (serially (fn [f] (f))
                                  (cond-> [self-test-law self-test-challenge]
                                    (seq page-entries)
                                    ;; Order is load-bearing in THIS register:
                                    ;; pick 0 is mof.home (a plain URL with no
                                    ;; declared redirect), whose http:// form
                                    ;; lands on the https URL, so the redirect
                                    ;; self-test's mismatch actually appears.
                                    ;; portal.home carries
                                    ;; :page/redirects-to, which would match
                                    ;; where it lands and swallow the mismatch
                                    ;; -- so it gets title and charset, which a
                                    ;; declared redirect does not touch.
                                    (into [#(self-test-redirect (pick 0))
                                           #(self-test-title (pick 1))
              ...
              (p/let [host-results (serially (fn [h] (p/let [r (check-host h)]
                ...
                (let [host-results (vec host-results)
                  ...
                  (cond
                    ...
                    ;; lasts. Observed 2026-08-29 on the sibling METI register:
                    ;; a correctly-caught charset drift sat in the output under
                    ;; FAIL in a run that exited 2 because another source had
                    ;; been challenged -- and the harness reading it then
                    ;; reported that the verifier does not discriminate, about a
                    ;; run whose own output shows it discriminating.
                    ;;
